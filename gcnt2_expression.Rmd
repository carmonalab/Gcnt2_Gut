...
```{r significance_plots}
ggplot2::ggplot(gcnt2.condition.expression, ggplot2::aes(x = Treatment, y = GCNT2)) +
  ggplot2::geom_jitter(width = 0.05, size = 1) +
  ggplot2::stat_summary(fun = mean, geom = "crossbar", width = 0.5,
                        fatten = 2, color = "magenta") +
  ggpubr::stat_compare_means(
    comparisons = list(combn(unique(gcnt2.condition.expression$Treatment), 2)),
    method = "wilcox.test",
    label = "p.signif") +
  ggplot2::xlab("Gut Condition") +
  ggplot2::ylab("GCNT2 Expression") +
  ggplot2::ggtitle("GCNT2 Expression vs. Gut Condition Omitting GCNT2 Non-expressing Cells, Wilcoxon Test") +
  ggplot2::theme_classic()
...
gcnt2.tukey.results[[1]] %>%
  ...
  tibble::rownames_to_column(var = "groups") %>%
  tidyr::separate(groups, into = c("group1", "group2"), sep = "-") %>%
  dplyr::mutate(y_pos = seq(from = 1.2 * max(gcnt2.condition.expression$GCNT2),
                            to = 1.2 * max(gcnt2.condition.expression$GCNT2) + 0.7*nrow(.),
                            length.out = nrow(.))) %>%
  dplyr::mutate(p.sig = case_when(
    p_adj <= 0.0001 ~ "****",
    p_adj <= 0.001 ~ "***",
...
ggplot2::ggplot(gcnt2.condition.expression, ggplot2::aes(x = Treatment, y = GCNT2)) +
  ...
  ggplot2::xlab("Gut Condition") +
  ggplot2::ylab("GCNT2 Expression") +
  ggplot2::ggtitle("GCNT2 Expression vs. Gut Condition Omitting GCNT2 Non-expressing Cells, Tukey Test") +
  ggplot2::theme_classic()
```
