...
```{r adding_meta_column}
cd4_meta <- cd4_cells@meta.data[, "functional.cluster", drop = FALSE]
cd8_meta <- cd8_cells@meta.data[, "functional.cluster", drop = FALSE]

cmc$functional.cluster <- cmc$scGate_multi 

cmc@meta.data[rownames(cd4_meta), "functional.cluster"] <- cd4_meta
cmc@meta.data[rownames(cd8_meta), "functional.cluster"] <- cd8_meta
```
