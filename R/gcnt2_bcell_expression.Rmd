...
Let us save our B cell, Seurat object that has been clustered.

```{r saving_bcells}
saveRDS(b_cells, file.path(output_data_path,paste0(ds_name,sprintf("_bcells_%s.rds", size))))
```
...
## Volcano Plot: Corrected

```{r volcano_plot_display_2}
de_genes$de_expression <- "NO"
de_genes$de_expression[de_genes$avg_log2FC > 0.6 
                                    & de_genes$p_val_adj < 0.05] <- "UP"
de_genes$de_expression[de_genes$avg_log2FC < -0.6 
                                    & de_genes$p_val_adj < 0.05] <- "DOWN"

de_genes$label <- NA
de_genes["GCNT2", "label"] <- "GCNT2"

top_DE_genes <- (de_genes$avg_log2FC > 1.75 & de_genes$p_val_adj < 1e-75) | 
                (de_genes$avg_log2FC > 5 & de_genes$p_val_adj < 0.05) |
                (de_genes$avg_log2FC < -5 & de_genes$p_val_adj < 1e-100) |
                (de_genes$avg_log2FC < -6.5 & de_genes$p_val_adj < 0.05) 
  
  

de_genes$label[top_DE_genes] <- rownames(de_genes)[top_DE_genes]

color_palette <- c("NO" = "black", "UP" = "green", "DOWN" = "red")

ggplot2::ggplot(de_genes, ggplot2::aes(x = avg_log2FC, y = -log10(p_val_adj), 
                                                    col = de_expression, label = label)) +
  ggplot2::geom_point(size = 0.3) +
  ggplot2::scale_color_manual(values = color_palette) +
  ggplot2::labs(color = "Differential Expression") +
  ggrepel::geom_label_repel() +
  ggplot2::geom_vline(xintercept=c(-0.6, 0.6), col="magenta") +
  ggplot2::geom_hline(yintercept=-log10(0.05), col="magenta") + 
  ggplot2::ggtitle("Volcano Plot, Cluster 1 vs. Cluster 2") +
  ggplot2::theme_classic()
```
...
```{r wilcoxon_tests}
multiple.wilcox.tests(cluster_treatment_binary_expression, "RNA_snn_res.0.07", "expression_ratio",
                      alternative = "less")
multiple.wilcox.tests(cluster_sample_binary_expression, "RNA_snn_res.0.07", "expression_ratio", 
                      alternative = "less")
```
...
```{r wilcoxon_tests}
multiple.wilcox.tests(b_cells@meta.data, "RNA_snn_res.0.07", "GCNT2",
                      alternative = "less")
```
