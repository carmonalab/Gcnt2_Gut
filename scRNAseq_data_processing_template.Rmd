...
# num_cells_i <- data.frame(patient = names(data_matrices), 
#                          cell_count = sapply(data_matrices, ncol))
# num_cells_i
# 
# 
# 
# for(key in names(data_matrices)){
#   sums <- Matrix::colSums(data_matrices[[key]])
#   ind <- which(sums < 10)
#   data_matrices[[key]] <- data_matrices[[key]][, -ind]
# }
# 
# cat("Final number of cells of patient 1: ", ncol(data_matrices[[1]]), "\n")
# num_cells_f <- data.frame(patient = names(data_matrices), 
#                          cell_count = sapply(data_matrices, ncol))
# num_cells_f
```
RDS files
...
```{r check_mtx}
cat("Total number of detected cells in raw dataset:", sum(sapply(data_matrices, ncol)), "\n")
cat("\n")
cat(dim(data_matrix), "\n")
...
data_matrices[[1]][1:10, 1:2]
print(head(colnames(data_matrices[[1]])))
cat("\n")
print(head(metas[[1]]))
print(head(rownames(metas[[1]])))
```
...
# 
# # rownames(meta) <- substr(rownames(meta), 17, length(rownames(meta)))
# 
...
for(key in names(data_matrices)){
  ...
  data_seurat <- Seurat::CreateSeuratObject(counts=data_matrices[[key]],
                                    ...
                                    min.cells = 3,
                                    min.features = 10)
  seurat_list[[key]] <- data_seurat

}
```

```{r merging_seurat_objects}
# Merge Seurat Objects
# require(Seurat)
names(seurat_list) <- NULL
data_seurat <- SeuratObject:::merge.Seurat(seurat_list[[1]], seurat_list[-1])
``` 


```{r freeing_memeory}
# free up rstudio memory
rm(data_matrix, meta, cell_ids, prefix, data_matrices, metas, seurat_list)
gc()
# 
...
data_seurat <- SeuratObject::JoinLayers(data_seurat, layers = "counts")
