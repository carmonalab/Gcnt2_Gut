...
for (i in unique(Seurat::Idents(cmc))){
  cat("Cluster", i, "\n")
  print(get(sprintf("cluster%.0f.markers", as.integer(i))))
}
```
...
for (i in unique(Seurat::Idents(cmc))){
  cluster_of_interest <- get(sprintf("cluster%.0f.markers", as.integer(i)))
  print(Seurat::FeaturePlot(cmc, features = top_DE_genes[[as.integer(i)]]) + ggtitle(paste(top_DE_genes[[as.integer(i)]], sprintf("Cluster %.0f", as.integer(i)), sep = " ")))
}
```
