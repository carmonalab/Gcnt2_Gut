...
for (i in unique(Seurat::Idents(cmc))){
  ...
  fake_row <- setNames(as.data.frame(matrix(rep(title, ncol(df)), nrow=1)), colnames(df))
  df_with_title <- rbind(fake_row, df)
  rownames(df_with_title)[1] <- title
  print(df_with_title)
}
```
...
for (i in unique(Seurat::Idents(cmc))){
  ...
  top_DE_genes[[as.integer(i)]] <- top_1_DE_gene
}
...
for (i in unique(Seurat::Idents(cmc))){
  cluster_of_interest <- get(sprintf("cluster%.0f.markers", as.integer(i)))
  print(Seurat::FeaturePlot(cmc, features = top_DE_genes[[as.integer(i)]]) + ggplot2::ggtitle(paste(top_DE_genes[[as.integer(i)]], sprintf("Cluster %.0f", as.integer(i)), sep = " ")))
}
```
...
cured_cmc <- scGate::scGate(cured_cmc, model = gating_model, ncores = 1)
...
clusters <- unique(Seurat::Idents(cured_cmc))

frequency_list <- list()

for (cluster in clusters){
  cluster_subset <- subset(cured_cmc, idents = cluster)
  cell_counts <- table(cluster_subset$scGate_multi)
  cell_percentages <- (cell_counts * 100) / sum(cell_counts)
  frequency_list[[as.character(cluster)]] <- cell_percentages
}

gating_df <- do.call(rbind, lapply(frequency_list, function(x) {
  all_types <- unique(unlist(lapply(frequency_list, names)))
  filled <- setNames(rep(0, length(all_types)), all_types)
  filled[names(x)] <- x
  return(filled)
}))

gating_df <- as.data.frame(gating_df)
rownames(gating_df) <- names(frequency_list)
